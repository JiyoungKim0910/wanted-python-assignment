...
from app.schemas import CompanyResponseSchema, TagNameSchema, TagUpdateSchema 
...
@router.get("/tags", response_model=List[dict] , summary="태그명으로 회사 검색", tags=["Tag"])
...
@router.put("/company/{name}/tags", response_model=CompanyResponseSchema, summary="회사 태그 정보 추가", tags=["Tag"])
...
@router.delete("/company/{name}/tags/{tagname}", response_model=CompanyResponseSchema, summary="회사 태그 정보 삭제", tags=["Tag"])
def delete_tag(
    name: str,
    tagname: str,
    x_wanted_language: str = Header("ko"),
